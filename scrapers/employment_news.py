...
class EmploymentNewsScraper(BaseScraper):
    """Scraper for EmploymentNews.gov.in"""
    ...
    def parse_jobs(self, html: str) -> List[Dict[str, Any]]:
        """Parse jobs from EmploymentNews HTML"""
        jobs = []
        ...
        try:
            ...
            # Find job list items
            job_list_selector = self.selectors.get('job_list', 'table.Grid tr')
            job_items = soup.select(job_list_selector)
            
            if not job_items:
                logger.warning("No job items found on EmploymentNews")
                return jobs
            
            for item in job_items:
                try:
                    # Extract title
                    title_selector = self.selectors.get('title', 'td:nth-child(2)')
            ...
            logger.info(f"Parsed {len(jobs)} jobs from EmploymentNews")
            return jobs
            
        except Exception as e:
            logger.error(f"Failed to parse EmploymentNews HTML: {e}")
            return []
