...
class SarkariResultScraper(BaseScraper):
    ...
    def parse_jobs(self, html: str) -> List[Dict[str, Any]]:
        """Parse jobs from SarkariResult HTML"""
        jobs = []
        ...
        try:
            ...
            # Find job list items
            job_list_selector = self.selectors.get('job_list', '.post a')
            job_items = soup.select(job_list_selector)
            
            if not job_items:
                logger.warning("No job items found on SarkariResult")
                return jobs
            
            for item in job_items:
                try:
                    # Extract title
                    title_selector = self.selectors.get('title', 'h2')
            ...
            logger.info(f"Parsed {len(jobs)} jobs from SarkariResult")
            return jobs
            
        except Exception as e:
            logger.error(f"Failed to parse SarkariResult HTML: {e}")
            return []
